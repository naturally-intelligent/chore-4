...
func fade_in_loop_sound(sound_name: String, volume:=1.0, fade_in_time:=1.5) -> void:
...
func fade_out_sound(sound_name: String, fade_out_time:=1.0) -> bool:
	...
	for container: Node in containers:
		for player: AudioStreamPlayer in container.get_children():
...
func _find_empty_sound_player() -> AudioStreamPlayer:
	for player: AudioStreamPlayer in $SoundPlayers.get_children():
		if not player.playing:
			return player
	return $SoundPlayers/SoundPlayer1

func _find_loaded_sound_player(resource_link: String) -> AudioStreamPlayer:
	for child: AudioStreamPlayer in $SoundPlayers.get_children():
		if child.has_meta('resource_link') and child.get_meta('resource_link') == resource_link:
			return child
	return null

func _is_sound_resource_playing(resource_link: String) -> bool:
	for player: AudioStreamPlayer in $SoundPlayers.get_children():
		if player.playing and player.has_meta('resource_link') and player.get_meta('resource_link') == resource_link:
			return true
	for player: AudioStreamPlayer in $SoundLoopers.get_children():
		if player.playing and player.has_meta('resource_link') and player.get_meta('resource_link') == resource_link:
			return true
	return false

func _find_empty_sound_looper() -> AudioStreamPlayer:
	for player: AudioStreamPlayer in $SoundLoopers.get_children():
		if not player.playing:
			return player
	return $SoundLoopers/SoundPlayer1

func _find_loaded_sound_looper(resource_link: String) -> AudioStreamPlayer:
	for player: AudioStreamPlayer in $SoundLoopers.get_children():
		if player.has_meta('resource_link') and player.get_meta('resource_link') == resource_link:
			return player
	return null

func _is_sound_resource_looping(resource_link: String) -> bool:
	for player: AudioStreamPlayer in $SoundLoopers.get_children():
		if player.playing and player.get_meta('resource_link') == resource_link:
			return true
	return false

func _stop_sound_resource(resource_link: String) -> bool:
	var stopped := false
	var containers := [$SoundPlayers, $SoundLoopers]
	for container: Node in containers:
		for player: AudioStreamPlayer in container.get_children():
...
func _fade_out_sound_player(player: AudioStreamPlayer, fade_out_time: float) -> void:
	var tween := create_tween()
...
func play_distant_sound(sound_name: String, base_volume:float, origin:Vector2, listener:Vector2) -> void:
...
func play_sound_if_not(sound_name: String, volume:=1.0) -> bool:
	if not is_sound_playing(sound_name):
		play_sound(sound_name, volume, false)
		return true
	return false

func delayed_sound(sound_name: String, time: float, volume:=1.0) -> void:
	if dev.silence: return
	# todo: more timers
...
func stop_all_sounds() -> void:
	for sound: AudioStreamPlayer in $SoundPlayers.get_children():
		sound.stop()
		sound.playing = false
	for looper: AudioStreamPlayer in $SoundLoopers.get_children():
		if not dev.silence:
...
func rogue(player: Object, load_with_sound_name:='') -> void:
...
func play_random_node(player: Object, sound_name: String, total: int):
	if dev.silence: return
	var c := math.random_int(1,total)
...
func force_music(song_name:String, volume:=1.0) -> void:
...
func resume_music(fade_in:=false) -> void:
...
func stop_and_reset_music() -> void:
	stop_music()
	set_music_volume(settings.music_volume)

func stop_music_animations() -> void:
	if fade_in_music_tween:
		fade_in_music_tween.kill()
	if fade_out_music_tween:
		fade_out_music_tween.kill()

func fade_in_music(song_name: String, _fade_in_time:=1.0, _do_play:=true, _stop_music:=true) -> void:
	if dev.no_music: return
...
func fade_out_music(_fade_out_time:=0.5) -> void:
...
func fade_out_in_music(song_name: String, _fade_out_time:=0.5, _fade_in_time:=1.0) -> void:
...
func is_playing_music_pool(pool_name: String) -> bool:
	if not pool_name in settings.music_pools:
		debug.print("WARNING: Missing Music Pool:", pool_name)
		return false
	for track_name: String in settings.music_pools[pool_name]:
...
func get_song_alias(song_name: String) -> String:
...
func seek_music(time: float) -> void:
	if $MusicPlayer.is_playing():
...
func _music_resource(song_name: String) -> String:
	...
	if song_name in settings.music_alias:
		song_name = settings.music_alias[song_name]
	for dir: String in settings.music_dirs:
...
func play_sound_if_ready(sound_name: String) -> void:
	if not root.switching_scene:
		play_sound(sound_name)

func play_slider_sound(slider_value: float, sound_name: String) -> void:
	play_sound(sound_name)

func button_sounds(button: Node, hover_sound:='menu-hover', press_sound:='menu-press') -> void:
...
func button_hover_sounds(button: Node, focus_sound:='menu-hover', unfocus_sound:='') -> void:
...
func calm_button_hover_sounds(button: Node, focus_sound:='menu-hover', unfocus_sound:='') -> void:
